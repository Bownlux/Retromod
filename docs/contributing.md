...
```bash
java -jar dist/CLI/retromod-1.3.0-snapshot.6-cli.jar --help
```
...
Upload only the 68 loader jars to loader-specific Modrinth or CurseForge versions. Publish `dist/CLI/retromod-1.3.0-snapshot.6-cli.jar` and `dist/SHA256SUMS.txt` with the GitHub release.
