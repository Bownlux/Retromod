...
Choose a jar from the matching loader and Minecraft folder. The release matrix contains 23 Fabric jars, 23 Forge jars, and 22 NeoForge jars. The separate `retromod-1.3.0-snapshot.7-cli.jar` is a command-line tool, not a mod, and does not belong in `mods/`.
