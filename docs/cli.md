...
Download `retromod-1.3.0-rc.1-cli.jar` from the GitHub release. It bundles its dependencies and has an executable manifest:

```bash
java -jar retromod-1.3.0-rc.1-cli.jar <command> <args>
```
...
```bash
# Inspect a mod
java -jar retromod-1.3.0-rc.1-cli.jar analyze path/to/mod.jar

# Transform one mod and use the exact target jar for safe Mixin repairs
java -jar retromod-1.3.0-rc.1-cli.jar transform path/to/mod.jar \
  --target 26.2 --mc-jar path/to/minecraft-26.2-client.jar --verify

# Transform a folder
java -jar retromod-1.3.0-rc.1-cli.jar batch path/to/mods \
  --aot --mc-jar path/to/minecraft-26.2-client.jar --verify

# Prepare a Minecraft instance
java -jar retromod-1.3.0-rc.1-cli.jar prepare path/to/.minecraft --aot

# Update one resource pack without launching Minecraft
java -jar retromod-1.3.0-rc.1-cli.jar resourcepack path/to/old-pack.zip \
  --target 26.2 --output path/to/updated-packs

# List registered shims
java -jar retromod-1.3.0-rc.1-cli.jar shims

# Compare two version points
java -jar retromod-1.3.0-rc.1-cli.jar diff fabric 1.21.1 26.2
```
...
- `--mc-jar <target.jar>` indexes the exact target Minecraft jar for conservative automatic Mixin translation and for checking whether a class can be extended on that version. `transform`, `aot`, and `batch` accept it. Without it, those commands look for an installed copy of the target version under PrismLauncher or `.minecraft/versions` and say which jar they used. Pass `--mc-jar` to decide that yourself.
...
If `--target` is omitted, `--mc-jar` also tries to infer the target from the jar's `version.json`. A standard filename such as `minecraft-26.2-client.jar` is the fallback. An explicit `--target` always wins. With no jar found either way, registered shims and mapping tables still run, but the exact target-method analysis is unavailable and the class-shape checks fall back to Retromod's own tables.
