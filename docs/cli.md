...
Download `retromod-1.3.1-cli.jar` from the GitHub release. It bundles its dependencies and has an executable manifest:

```bash
java -jar retromod-1.3.1-cli.jar <command> <args>
```
...
```bash
# Inspect a mod
java -jar retromod-1.3.1-cli.jar analyze path/to/mod.jar

# Transform one mod and use the exact target jar for safe Mixin repairs
java -jar retromod-1.3.1-cli.jar transform path/to/mod.jar \
  --target 26.2 --mc-jar path/to/minecraft-26.2-client.jar --verify

# Transform a folder
java -jar retromod-1.3.1-cli.jar batch path/to/mods \
  --aot --mc-jar path/to/minecraft-26.2-client.jar --verify

# Prepare a Minecraft instance
java -jar retromod-1.3.1-cli.jar prepare path/to/.minecraft --aot

# Update one resource pack without launching Minecraft
java -jar retromod-1.3.1-cli.jar resourcepack path/to/old-pack.zip \
  --target 26.2 --output path/to/updated-packs

# List registered shims
java -jar retromod-1.3.1-cli.jar shims

# Compare two version points
java -jar retromod-1.3.1-cli.jar diff fabric 1.21.1 26.2
```

Run `--help` for the full command and option list:

```bash
java -jar retromod-1.3.1-cli.jar --help
```
